...
Layout written to the device:
    ...
    /overlay/seedsigner/...   <- overlay dev lane (--mode overlay); shadows the FROZEN app

The /lib push needs no sys.path edit (device sys.path already includes '/lib'), but
on a FROZEN firmware /lib is DEAD -- `.frozen` precedes it, so the frozen app always
wins. To iterate on app code against a frozen firmware WITHOUT a rebuild, use the
overlay lane: it pushes the whole `seedsigner` package to /overlay/seedsigner, which
the launcher (_launcher.MAIN_PY) puts ahead of `.frozen` via `sys.path.insert(0,
'/overlay')`. See docs/knowledge/micropython-frozen-vs-vfs-override.md.

Modes:
    --mode push-only      push source to /lib, touch nothing else (default)
    --mode import-smoke    push to /lib, then `import seedsigner.controller` on-device
                           and report success / full traceback (no reset, no main.py)
    --mode run            push to /lib, write /main.py launcher, reset
    --mode overlay        push `seedsigner` to /overlay/seedsigner (shadows the frozen
                           app), bake /overlay/seedsigner/_version.py, write the
                           launcher, reset -- the fast frozen-firmware dev lane
    --mode overlay-clear  delete /overlay/seedsigner + reset -> next boot runs the
                           frozen app again (no push)
...
import _version_bake  # noqa: E402  (shared version determination + _version.py rendering)
from _launcher import MAIN_PY  # noqa: E402  (one launcher definition, shared by all writers)
...
# Overlay dev lane (Part 3 of the structured-frozen-build design): a whole
# `seedsigner` package pushed to /overlay/seedsigner shadows the FROZEN copy,
# because the launcher (_launcher.MAIN_PY) prepends /overlay to sys.path ahead of
# .frozen. Overrides ONLY seedsigner -- embit/urtypes stay frozen (and fast). See
# docs/knowledge/micropython-frozen-vs-vfs-override.md.
OVERLAY_ROOT = "/overlay"
OVERLAY_SS_DST = OVERLAY_ROOT + "/seedsigner"
...
def device_tree(ser, roots=(SS_DST, EMBIT_DST)):
    """One recursive os.stat pass; returns {path: size} for the given dst roots
    (the /lib app+embit roots by default; /overlay/seedsigner for the overlay lane)."""
    calls = "; ".join("_ls('%s')" % r for r in roots)
    out = raw_exec(ser, "%s; print('LS_END')" % calls, timeout=30)
    on_dev = {}
...
def verify(ser, expected, roots=(SS_DST, EMBIT_DST)):
    on_dev = device_tree(ser, roots)
...
def write_launcher(ser):
    """Write the shared launcher (_launcher.MAIN_PY) to /main.py so the app
    auto-runs at boot. Used by --mode run and --mode overlay; the launcher
    prepends /overlay to sys.path, so one definition serves both the frozen and
    overlay lanes."""
    ...
    print("[deploy] wrote /main.py launcher (%d bytes)" % len(MAIN_PY.encode()))


def bake_overlay_version(ser):
    """Resolve the version (env override -> app git, via the shared _version_bake
    helper) and push a fresh /overlay/seedsigner/_version.py -- the on-device
    analogue of what the frozen stager bakes into frozen_app/seedsigner/_version.py.
    Because _version.py is a MEMBER of the package, it travels with whichever copy
    wins, so an overlaid app reports the OVERLAY version (not the frozen one) via
    the same `from seedsigner import _version` line. The version is resolved from
    the configured app dir (_devenv.SS_APP_DIR); set $SEEDSIGNER_VERSION to stamp
    an explicit override. A stale _version.mpy (from a prior overlay) is removed so
    it can't shadow this .py."""
    version, source = _version_bake.resolve_version(_devenv.SS_APP_DIR, _devenv.SS_SRC_ROOT)
    text = _version_bake.render_version_module(version)
    raw_exec(ser, "_rmrf('%s/_version.mpy')" % OVERLAY_SS_DST)  # drop any stale bytecode
    push_bytes(ser, text.encode(), OVERLAY_SS_DST + "/_version.py")
    print("[deploy] overlay version (%s): %s  ->  %s/_version.py"
          % (source, version.get(_version_bake.KEY_NAME), OVERLAY_SS_DST))


def deploy_overlay(ser, args, compile_mpy, mpy_cross):
    """Part 3 dev lane: mirror the whole `seedsigner` package to /overlay/seedsigner
    (shadowing the frozen copy since the launcher prepends /overlay to sys.path),
    bake its _version.py, ensure the launcher is present, then reset to boot it.
    Overrides ONLY seedsigner -- embit/urtypes stay frozen (and fast)."""
    roots = (OVERLAY_SS_DST,)
    if args.clean:
        print("[deploy] cleaning %s ..." % OVERLAY_SS_DST)
        raw_exec(ser, "_rmrf('%s'); print('cleaned')" % OVERLAY_SS_DST)
    ss_excl = {"__pycache__"} if args.resources else {"__pycache__", "resources"}

    # Same auto-retry-against-CDC-truncation loop as the /lib push, over the single
    # overlay root.
    ok = False
    for attempt in range(1, PUSH_ATTEMPTS + 1):
        expected = {}
        on_dev = {} if (args.clean and attempt == 1) else device_tree(ser, roots)
        force = args.force and attempt == 1
        try:
            push_tree(ser, args.seedsigner_src, OVERLAY_SS_DST, "seedsigner", ss_excl,
                      (".pyc",), expected, on_dev, force,
                      compile_mpy=compile_mpy, mpy_cross=mpy_cross)
            ok = verify(ser, expected, roots)
    ...
    if not ok:
        sys.exit("[deploy] verify: FAIL after %d attempts — refusing to boot a "
                 "partial/truncated overlay. Check the USB-CDC link and re-run." % PUSH_ATTEMPTS)

    bake_overlay_version(ser)
    write_launcher(ser)
    print("[deploy] overlay deployed. Resetting to boot the overlaid app ...")
    ser.write(b"\r\x03\x03import machine; machine.reset()\r")


def clear_overlay(ser):
    """Delete /overlay/seedsigner so the next boot resolves the FROZEN app again.
    The launcher stays in place: with /overlay empty it costs one failed stat and
    the frozen app runs."""
    print("[deploy] clearing %s (next boot runs the frozen app) ..." % OVERLAY_SS_DST)
    raw_exec(ser, "_rmrf('%s'); print('cleared')" % OVERLAY_SS_DST)
    print("[deploy] resetting ...")
    ser.write(b"\r\x03\x03import machine; machine.reset()\r")
...
def main():
    ...
    ap.add_argument("--clean", action="store_true",
                    help="recursively remove the destination first (/lib app+embit, or "
                         "/overlay/seedsigner in --mode overlay)")
    ...
    mpy_cross = None
    no_push_modes = ("smoke-only", "overlay-clear")  # these push no files
    if compile_mpy and args.mode not in no_push_modes:
    ...
    try:
        ...
        # overlay-clear: just drop /overlay/seedsigner + reset (no push).
        if args.mode == "overlay-clear":
            clear_overlay(ser)
            print("[deploy] done.")
            return

        # overlay: push seedsigner to /overlay/seedsigner (shadows the frozen copy),
        # bake _version, write the launcher, reset. Does NOT touch /lib or embit.
        if args.mode == "overlay":
            deploy_overlay(ser, args, compile_mpy, mpy_cross)
            print("[deploy] done.")
            return
        ...
        if args.mode == "import-smoke":
            ...
            print("[deploy] writing /main.py to boot Controller ...")
            write_launcher(ser)
    finally:
        ser.close()
    print("[deploy] done.")
